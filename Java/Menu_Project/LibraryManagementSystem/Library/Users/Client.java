        ...
        return () -> {
            ...
            while (true) {
                ...
                while (name.length() <= 0) {
                    ...
                    if (name.length() <= 0) {
                        System.out
                                .println(ConsoleColors.RED + "Please Enter Name For The Client" + ConsoleColors.RESET);
                    }
                }

                while (!Validation.isValidEmail(email)) {
                    System.out.print(ConsoleColors.CYAN + "Please Enter Email For The Client : " + ConsoleColors.BLUE);

                    email = console.readLine();

                    if (!Validation.isValidEmail(email)) {
                        System.out
                                .println(ConsoleColors.RED + "Please Enter Valid Email For The Client"
                                        + ConsoleColors.RESET);
                    }
                }
